...
def _ListNodesAtPath(path):
  """Returns a list of configured nodes at path. LDAPv3/BSD/Local etc."""
  command = [_DSCL, 'localhost', 'list', path]
  (stdout, stderr, returncode) = RunProcess(command)
  if returncode:
    raise DSException('Unable to list nodes at %s '
                      'Error: %s' % (path, stderr))
  nodes = stdout.strip().split()
  return [ '%s/%s' % (path, node) for node in nodes ]
...
def GetLDAPv3SearchNodes():
  """Returns LDAPv3 search nodes for DS /Search path."""
  nodes = GetSearchNodes()
  return [node for node in nodes if node.startswith('/LDAPv3')]


def GetLocalSearchNodes():
  """Returns Local search nodes for DS /Search path."""
  nodes = GetSearchNodes()
  return [node for node in nodes if node.startswith('/Local')]


def GetBSDSearchNodes():
  """Returns BSD search nodes for DS /Search path."""
  nodes = GetSearchNodes()
  return [node for node in nodes if node.startswith('/BSD')]


def GetADSearchNodes():
  """Returns AD search nodes for DS /Search path."""
  nodes = GetSearchNodes()
  return [node for node in nodes if node.startswith('/Active Directory')]
...
def GetLDAPv3ContactsNodes():
  """Returns LDAPv3 contacts nodes for DS /Search path."""
  nodes = GetContactsNodes()
  return [node for node in nodes if node.startswith('/LDAPv3')]


def GetLocalContactsNodes():
  """Returns Local contacts nodes for DS /Search path."""
  nodes = GetContactsNodes()
  return [node for node in nodes if node.startswith('/Local')]


def GetBSDContactsNodes():
  """Returns BSD contacts nodes for DS /Search path."""
  nodes = GetContactsNodes()
  return [node for node in nodes if node.startswith('/BSD')]


def GetADContactsNodes():
  """Returns AD contacts nodes for DS /Search path."""
  nodes = GetContactsNodes()
  return [node for node in nodes if node.startswith('/Active Directory')]
...
def ConfiguredNodesLocal():
  """Returns list of configured nodes in /Local."""
  return _ListNodesAtPath('/Local')


def ConfiguredNodesBSD():
  """Returns list of configured nodes in /BSD."""
  return _ListNodesAtPath('/BSD')


def ConfiguredNodesLDAPv3():
  """Returns list of configured nodes in /LDAPv3."""
  return _ListNodesAtPath('/LDAPv3')


def ConfiguredNodesAD():
  """Returns list of configured nodes in /Active Directory."""
  return _ListNodesAtPath('/Active Directory')
