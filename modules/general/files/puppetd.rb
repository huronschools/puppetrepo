...
$linux_uid = "#{Facter.value(:serialnumber).downcase()}.#{$suffix}" if Facter.value(:operatingsystem) == 'CentOS'
...
def setPuppetConf()
  ...
  if Facter.value(:operatingsystem) == 'CentOS'
    confHash = {"server" => $server, "certname" => $linux_uid, "pluginsync" => "true", "factpath" => "$vardir/lib/facter", "report" => "true"}
...
if ARGV[0] == "-v"
  ...
  ## Call Operating System Specific Code
  if Facter.value(:operatingsystem) == 'CentOS'
    runLinuxWrapper("verbose")
  elsif Facter.value(:operatingsystem) == 'Darwin'
    runOSXWrapper("verbose")
  else
    puts 'Sorry, Operating System Not Supported'
    exit(1)
  end 
else
  preflight()
